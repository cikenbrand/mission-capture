...
| [1](phase-1-shell-and-layers.md) | Shell & Layers tree | 11 | **8** | `wip` |
...
**Acceptance criteria met:** 13 / 113 — `P0-AC1`–`AC5` and `P1-AC1`, `AC2`, `AC3`, `AC7`–`AC11`, each asserted by T0 or T1 with evidence in their run reports.

Phase 0 and Phase 1 define 13 criteria each; the later phases have not had theirs written yet, which is why the denominator is a plan figure rather than a measured one. Phase 0's remaining eight and Phase 1's remaining five (`AC4`, `AC5`, `AC6`, `AC12`, `AC13` — the recording-safety ones) are untested, not failing.
...
| 1.9a | Recording-safety UI | `todo` | | | Record indicator, confirm-on-stop, locked mode. Split out 2026-08-13 — all frontend, none of it able to damage a recording |
| 1.9b | Disk-space protection | `todo` | | | Status-bar free space (a gap the 1.5 audit raised), warnings at 10 GB and 2 GB, hard stop with a **clean file close**. Own task because it reaches into the output pipeline, and getting it wrong produces exactly the unplayable file 1.7's MKV default exists to prevent. Needs a fixture that does not involve filling a real disk |
| 1.9c | Unattended startup | `todo` | | | Closes OI-46 and OI-54, which are one decision rather than two. **Blocked on the user choosing a policy** for what the app does when it starts with nobody watching |
