...
| [2](phase-2-video-elements.md) | Video elements | 6 | **6** | **`done`** |
...
| OI-4 | planning | question | med | Real channel count and data rate, and the RS-232 line settings actually in use | 3.1, 5.1 | ✅ **answered 2026-08-13.** Hard cap of **4 RS-232 devices**, operator's choice which; **10 Hz** each as an *expected* rate the engine measures against, so a device delivering 3 Hz can be called broken; **per-port** COM/baud/data bits/stop bits/parity, because four devices on a vessel will not agree; payload is **positional comma-separated floats**, with meaning supplied by an operator-configured index→channel map rather than by the parser — index 0 → CP, 1 → KP, 2 → Temperature. The parser extracts by position and must not know what the numbers mean, because every survey system orders its fields differently. Consequence worth keeping: 40 frames/second total is far below where a mutex-protected registry needs replacing, so the hot path stays simple by evidence rather than by hope. Re-confirm the transport side at Phase 5 |
...
| OI-58 | 1.8 | bug | med | **Every wizard-created Job keeps a stray empty Canvas.** `obs_frontend_add_scene_collection()` creates the collection with a default scene, and the wizard adds its Canvases alongside it rather than replacing it — so a Job set up for three cameras opens with four Canvases, one of them empty and named `Canvas`. Cosmetic but immediately visible, and the operator has to notice and delete it. Found by the user inspecting a real Job they had created. Fix in 2.2: reuse the default Canvas for the first camera, or remove it once the named ones exist | — | ✅ **closed 2026-08-13** — the wizard notes the default Canvas before adding its own and removes it after, switching to the first named Canvas first so the Job is never momentarily empty (which would make the frontend pick a new program Canvas and fire a scene change through the Layers tree) |
| OI-59 | 2.2 | risk | med | **Canvas-to-source resolution matching (OI-48) is implemented but never executed.** `matchCanvasToSource` polls a new Element six times over three seconds and resizes the Canvas to the camera's own resolution, replacing OBS's monitor-derived guess — but with no capture card on the development machine the source reports 0x0 and the function returns without acting every time. The arithmetic and the config-write path are unverified. **2.4 is the natural place to close it**: an RTSP fixture is a source with a known, controllable resolution, which is a better test vehicle than a capture card anyway | — | ✅ **closed 2026-08-13.** Observed: Canvas resized 1920x1080 → 1280x720 to match a live 720p source, 24 ms after first signal. Two changes made it real — the single-Canvas/single-Element guard the header had promised but the code never had, and hooking the resize to *first signal* rather than only to Add Element, since a Job loaded from disk never goes through that path. Now a T2 assertion |
| OI-60 | 2.3 | deferred | med | **No preview banner.** The plan asks for "Signal lost — reconnecting…" drawn across the Element's area in the preview. Built instead: a red dot and red name on the Element's row in the Layers tree, which I own and which carries no merge risk. A real preview overlay means drawing inside OBS's render callback in `OBSBasicPreview` — an upstream file this fork has never touched — and Qt widgets cannot reliably be layered over a native display surface. **Task 2.6 owns per-Element health display** and is the right place to do it once, properly, for all the states it will then have | 2.6 | open |
| OI-61 | 2.3 | deferred | med | **No reconnect with backoff of our own.** Both backends already retry internally — `win-dshow` has a reactivate callback, `decklink` re-Activates when its device returns — so a camera that comes back does recover without user action. What is missing is *our* policy on top: a bounded backoff, and a distinction between a pulled cable and a reset card, which the phase doc calls out as different recovery paths. Detection is in place and is the hard half; the policy needs a device that can actually drop to design against | 2.6 or a field session | open |
| OI-62 | 2.3 | risk | **high** | **The Lost transition has never executed.** The state machine only reports `unknown` here: the fixture camera never connects, and the one real device on this machine never dropped during a run. So `Receiving → Lost → Receiving`, the log lines either side, and the Layers marker are all **written but never observed**. This is the core of the task and it is unverified. **2.4 closes it directly** — a killable RTSP source drops on command, which is a better test than pulling a cable would be because it can be scripted | — | ✅ **closed 2026-08-13.** ffmpeg serving a killable TCP stream: frames counted at exactly 25/s, stream killed, `SIGNAL LOST` logged 3.0 s later — matching the threshold. Now an automated T2 assertion, so it cannot regress silently |
| OI-63 | 2.6 | deferred | med | **Per-Element latency is not measured.** The plan asks for glass-to-glass-ish latency, capture timestamp to render. The frame counter gives rate, not delay, and a real figure needs the frame's own timestamp compared against the clock at render — reachable from the watch filter, which already sees every frame, but it is a second mechanism rather than an extra column. The honest fallback the plan allows, the source's reported buffering, is 0 for every Element by our own defaults and would therefore display a confident zero that means nothing. Left unbuilt rather than faked | later | open |
...
| 2.6 | Latency and health measurement | **`done`** | 2026-08-13 | T2 at 64 assertions: Health dock registered and hidden by default, per-Element fps reported and zero for an absent camera | Health panel replaces upstream's Stats dock, which the 1.5 audit deferred here precisely so it could be built once against real per-Element data. **Frame rate is measured, not declared** — a camera configured for 25 and delivering 12 looks completely normal in the preview and is the fault found in review rather than on the dive. Hidden by default like the Stats dock it replaces, because the Layers tree already flags a lost camera unasked. **Also closed OI-59/OI-48** — the Canvas now sizes itself to the camera, verified against a live 720p source. **Latency is not implemented — OI-63**, and OI-60 and OI-61 remain open by choice |
