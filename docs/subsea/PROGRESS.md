...
| OI-51 | 1.8 | debt | med | **The Job menu could not retire with the Rig menu.** 1.8 replaces *creating* a Job but not *switching* between existing ones, and Scene Collection's list is the only route to that today. So `SceneCollectionMenu` stays ON while `ProfileMenu` goes off. Needs a Job switcher — a recent-Jobs list in File, or a Jobs panel — before the menu can go. Until then the vocabulary is inconsistent: one concept, one retired menu and one kept | 1.9 or later | open |
