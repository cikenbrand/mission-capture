...
| OI-4 | planning | question | med | Real channel count and data rate, and the RS-232 line settings actually in use | 3.1, 5.1 | ✅ **answered 2026-08-13.** Hard cap of **4 RS-232 devices**, operator's choice which; **1–10 Hz** each; **per-port** COM/baud/data bits/stop bits/parity, because four devices on a vessel will not agree; payload is **positional comma-separated floats** (heading, CP, northing, easting…). Consequence worth keeping: 40 frames/second total is far below where a mutex-protected registry needs replacing, so the hot path stays simple by evidence rather than by hope. Re-confirm the transport side at Phase 5 |
