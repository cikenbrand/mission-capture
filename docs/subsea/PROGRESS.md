...
| OI-38 | 1.3 | bug | med | **The program-Canvas marker is never invalidated.** `ProgramRole` is read during paint but nothing connects to a scene-changed event, so the red dot only moves when the dock happens to repaint for another reason. Observed directly: it looked correct in testing purely by luck of repaint. 1.4 wires selection sync and should emit `dataChanged` for the old and new program Canvas | 1.4 | open |
| OI-39 | 1.3 | risk | med | **The Job-switch path is unverified.** `ClearSceneData()` removes every scene and reload recreates them, so a Job switch drives dozens of `source_remove`/`source_create` signals through the model at once. Runtime *add* is verified; bulk teardown and rebuild is not, and nothing calls `reload()` on a collection change. First thing to check in 1.4 | 1.4 | open |
