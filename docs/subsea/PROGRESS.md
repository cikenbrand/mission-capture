...
| OI-28 | 0.6 | bug | **high** | **CI runs and has always failed** — the original wording (`never run`) was wrong; there are 20+ runs, all red, unwatched since 0.6. **Format is now green** (six files failed `clang-format`; three mine, three upstream files my edits broke). **Build still fails at Configure in ~3s** — too fast for real work, so CMake bails before doing anything. Three hypotheses tested and disproved: preset inheritance of the VS instance pin, the pinned Windows SDK 10.0.26100, and a poisoned dependency cache. Log text needs auth; `gh` could not be installed (the release-asset host is unreachable from this environment, and winget fails the same way) | release confidence | **open — needs the Configure log** |
